...
const DiningHall = ({ navigation, route }) => {
    ...
    const fetchComments = () => {
        ...
        fetch(
            `https://sleepy-reaches-22563.herokuapp.com/api/getAll/diningComments/${id}`,
            requestOptions
        )
            .then(async (response) => {
        ...
        <TouchableWithoutFeedback onPress={closeKeyboardAndMenu} accessible={false}>
            <SafeAreaView style={diningHallStyles.container}>
                <View style={diningHallStyles.header}>
                    ...
                    <StatusIcon
                        isOpen={true}
                        onPress={() => alert("omw to hours")}
                        style={diningHallStyles.statusIcon}
                    />
                </View>
                ...
                <Button
                    fontSize={16}
                    imgSrc={require("../../assets/images/gold-food.png")}
                    onPress={() => navigation.navigate("Menus", { diningHallId: id })}
                    style={diningHallStyles.menuButton}
                    text="View Menu"
                />
                ...
                {isRatingInputVisible && (
                    <RatingInput
                        diningHallId={id}
                        onClose={handleRatingInputClose}
                        userId={userId}
                    />
                )}
            </SafeAreaView>
