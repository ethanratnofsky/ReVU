...
const RatingInput = ({ diningHallId, onClose, userId }) => {
    ...
    useEffect(() => {
        fetch(
            `https://sleepy-reaches-22563.herokuapp.com/api/getAll/userRatings/${userId}`,
            requestOptions
        )
            .then(async (response) => {
    ...
    const updateRating = (ratingType) => {
        ...
        fetch(
            "https://sleepy-reaches-22563.herokuapp.com/api/patch/updateRating",
            updateRatingOptions
        )
            .then(async (response) => {
                const isJson = response.headers.get("content-type")?.includes("application/json");
        ...
        <SafeAreaView style={ratingInputStyles.container}>
            ...
            <View style={ratingInputStyles.ratingContainer}>
                ...
                <View style={ratingInputStyles.starsContainer}>
                    {[...Array(5)].map((_, index) => (
                        <TouchableOpacity
                            style={ratingInputStyles.starContainer}
                            key={index}
                            onPress={() => setFoodRating(index + 1)}
                        >
                            {index < foodRating && (
                                <Image
                ...
                <View style={ratingInputStyles.starsContainer}>
                    {[...Array(5)].map((_, index) => (
                        <TouchableOpacity
                            style={ratingInputStyles.starContainer}
                            key={index}
                            onPress={() => setTrafficRating(index + 1)}
                        >
                            {index < trafficRating && (
                                <Image
                                    style={ratingInputStyles.star}
            ...
            <TouchableOpacity
                style={{
                    ...ratingInputStyles.submitButtonContainer,
                    opacity: foodRating && trafficRating ? 1 : 0.25,
                }}
                onPress={handleSubmit}
                disabled={!(foodRating && trafficRating)}
            >
                <LinearGradient
                    colors={[VU_METALLIC_GOLD_START, VU_METALLIC_GOLD_END]}
                    start={{ x: 0, y: 1 }}
                    end={{ x: 1, y: 0 }}
                    style={ratingInputStyles.gradient}
                >
            ...
            <TouchableOpacity style={ratingInputStyles.closeButtonContainer} onPress={onClose}>
                <Image
                    style={ratingInputStyles.closeButton}
                    onPress={onClose}
                    source={require("../../assets/images/white-close.png")}
                />
            </TouchableOpacity>
        </SafeAreaView>
    );
