...
const CommentInput = ({ diningHallId, onSubmit, userId }) => {
    ...
    const handleSubmit = () => {
        if (filter.isProfane(comment)) {
            ...
            fetch(
                "https://sleepy-reaches-22563.herokuapp.com/api/post/createComment",
                requestOptions
            )
                .then(async (response) => {
    ...
    return (
        <KeyboardAvoidingView behavior="padding">
            <View style={commentInputStyles.container}>
                ...
                />
                <TouchableOpacity
                    style={commentInputStyles.sendContainer}
                    onPress={handleSubmit}
                    disabled={!comment}
                >
                    <Image
                        style={commentInputStyles.send}
                        source={
                            comment
                                ? require("../../assets/images/gold-send.png")
                                : require("../../assets/images/gray-send.png")
                        }
                    />
                </TouchableOpacity>
            </View>
