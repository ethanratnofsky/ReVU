...
const Subratings = ({ diningHallId }) => {
    ...
    const foodRatings = RATINGS.filter(
        (rating) => rating.diningHallId === diningHallId && rating.type === "food"
    );
    const trafficRatings = RATINGS.filter(
        (rating) => rating.diningHallId === diningHallId && rating.type === "traffic"
    );
    ...
    const foodRating =
        foodRatings.length === 0
            ? "N/A"
            : (
                  foodRatings.reduce((acc, rating) => acc + rating.rating, 0) / foodRatings.length
              ).toFixed(1);
    const trafficRating =
        trafficRatings.length === 0
            ? "N/A"
            : (
                  trafficRatings.reduce((acc, rating) => acc + rating.rating, 0) /
                  trafficRatings.length
              ).toFixed(1);
    ...
    useEffect(() => {
        fetch(
            `https://sleepy-reaches-22563.herokuapp.com/api/getAll/ratings/${diningHallId}`,
            requestOptions
        )
            .then(async (response) => {
    ...
    return (
        <View style={subratingsStyles.container}>
            <View
                style={
                    isKeyboardShown
                        ? subratingsStyles.subratingSmallContainer
                        : subratingsStyles.subratingContainer
                }
            >
                ...
                <View style={subratingsStyles.ratingContainer}>
                    <Text
                        style={
                            isKeyboardShown ? subratingsStyles.smallRating : subratingsStyles.rating
                        }
                    >
                        {foodRatingAvg}
                    </Text>
                    <Image
                        style={isKeyboardShown ? subratingsStyles.smallStar : subratingsStyles.star}
